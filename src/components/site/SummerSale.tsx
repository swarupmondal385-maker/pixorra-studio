...
export function SummerSale() {
  ...
  return (
    <>
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 bg-background/60 backdrop-blur-sm transition-opacity duration-300" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-full sm:max-w-sm glass-strong border-l-2 border-ink shadow-pixel transition-transform duration-500 ease-out ${sidebarOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="h-full flex flex-col overflow-y-auto">
          <div className="relative bg-pixel-orange pt-12 pb-6 px-5 sm:px-6">
            ...
            <button onClick={() => setSidebarOpen(false)} className="absolute top-3 right-4 h-8 w-8 rounded-full bg-card border-2 border-ink flex items-center justify-center hover:scale-110 transition-transform z-10" aria-label="Close sale">
            ...
            <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2">
              {[{ val: pads(left.days), label: "Days" }, { val: pads(left.hours), label: "Hrs" }, { val: pads(left.minutes), label: "Mins" }, { val: pads(left.seconds), label: "Secs" }].map((u) => (
                <div key={u.label} className="text-center">
                  <div className="h-11 rounded-xl bg-foreground text-background font-pixel text-base flex items-center justify-center border-2 border-ink">{u.val}</div>
