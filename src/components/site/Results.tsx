...
const RESULTS = [
  {
    biz: "Mumbai Dental Clinic",
    service: "Website + Google Ads",
    stat: "3× more bookings",
    period: "in 60 days",
    quote:
      "We went from 4 enquiries a week to 4 a day. The new site and Google campaign completely changed our clinic.",
    name: "Dr. Ankit S.",
    c: "bg-pixel-pink",
    emoji: "🦷",
  },
  {
    ...
    stat: "₹0 → ₹2L/mo",
    period: "online revenue",
    quote:
      "Pixorra didn't just make a website — they made me a real online business. Members literally sign up at midnight.",
    name: "Rahul M.",
    c: "bg-pixel-yellow",
    emoji: "💪",
  },
  {
    biz: "Surat D2C Jewellery",
    service: "Shopify + Ads",
    stat: "5.8× ROAS",
    period: "on Meta & Google",
    quote:
      "Clean store, fast checkout, and ads that actually pay back. Best decision we took this year.",
    name: "Priya & Kunal",
    c: "bg-pixel-cyan",
    emoji: "💎",
  },
  {
    ...
    stat: "Ranked #1",
    period: "for 12 local keywords",
    quote:
      "I used to cold-call clients. Now clients call me. The SEO work paid for itself in the second month.",
    name: "Suman B.",
    c: "bg-pixel-purple",
    emoji: "📊",
  },
  ...
  return (
    <section id="work" className="py-20 md:py-32 bg-cream relative overflow-hidden">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0">
        ...
        <div
          className="absolute bottom-10 -right-16 h-72 w-72 bg-pixel-cyan/25 blob"
      ...
      <div className="relative max-w-7xl mx-auto px-5 md:px-8">
        <div className="max-w-3xl reveal">
          ...
          <p className="mt-4 text-ink/65 text-lg">
            Not vanity metrics. Bookings, revenue, rankings — the stuff that actually pays the
            bills.
          </p>
        </div>
        ...
        <div className="mt-14 grid md:grid-cols-2 gap-6">
          {RESULTS.map((r, i) => (
            ...
            >
              {/* corner sticker */}
              <div
                className={`absolute -top-4 -left-4 h-14 w-14 rounded-2xl ${r.c} border-2 border-ink shadow-pixel flex items-center justify-center text-2xl wobble`}
              >
                {r.emoji}
              </div>
              ...
              <div className="flex items-start justify-between gap-4 pl-10">
                <div>
                  <div className="text-xs uppercase tracking-wider font-bold text-ink/50">
                    {r.biz}
                  </div>
              ...
              <div className="mt-6 inline-flex items-center gap-3 rounded-2xl border-2 border-ink bg-pixel-yellow/40 px-4 py-3">
                ...
                <div>
                  <div className="font-display text-3xl md:text-4xl font-bold text-ink tracking-tight leading-none">
                    {r.stat}
                  </div>
                  <div className="mt-1 text-xs text-ink/65 font-semibold uppercase tracking-wider">
                    {r.period}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 flex items-center gap-3">
                <div
                  className={`h-10 w-10 rounded-full ${r.c} border-2 border-ink text-white flex items-center justify-center text-xs font-bold`}
                >
                  {r.name
                    .split(" ")
                    .map((x) => x[0])
