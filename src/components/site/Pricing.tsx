...
const WHATSAPP =
  "https://wa.me/918927840261?text=Hi%20Pixorra%2C%20I%27d%20like%20to%20know%20more%20about%20your%20pricing.";
...
export function Pricing() {
  ...
  return (
    <section id="pricing" className="relative py-20 md:py-32 bg-background overflow-hidden">
      <div
        className="absolute -top-24 -left-20 h-80 w-80 rounded-full bg-pixel-pink/20 blur-3xl blob"
        aria-hidden
      />
      <div
        className="absolute top-40 -right-24 h-96 w-96 rounded-full bg-pixel-cyan/20 blur-3xl blob"
        style={{ animationDelay: "3s" }}
        aria-hidden
      />
      <div
        className="absolute bottom-10 left-1/3 h-72 w-72 rounded-full bg-pixel-yellow/25 blur-3xl blob"
        style={{ animationDelay: "5s" }}
        aria-hidden
      />
      <div className="absolute top-16 left-10 h-3 w-3 bg-pixel-pink sparkle-pulse" aria-hidden />
      <div
        className="absolute top-32 right-24 h-2.5 w-2.5 bg-pixel-purple sparkle-pulse"
        style={{ animationDelay: "1s" }}
        aria-hidden
      />
      <div
        className="absolute bottom-24 right-10 h-3 w-3 bg-pixel-green sparkle-pulse"
      ...
      <div className="relative max-w-7xl mx-auto px-5 md:px-8">
        <div className="max-w-3xl mx-auto text-center reveal">
          ...
          <p className="mt-5 text-lg text-ink/60">
            Three one-time plans built to grow your business. From Ignite to Elite — there's a
            growth engine for every stage.
          </p>
        </div>
        ...
        >
          {PACKAGES.map(
            (
              { name, price, originalPrice, period, tagline, features, cta, popular, flagship },
              i,
            ) => (
              <div
                key={name}
              ...
              >
                ...
                <div>
                  <h4
                    className={`font-display text-2xl font-bold ${flagship ? "text-ink" : "text-ink"}`}
                  >
                    {name}
                  </h4>
                ...
                <div className="mt-4 md:mt-6">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className={`text-sm ${flagship ? "text-ink/60" : "text-ink/50"}`}>
                      {period}
                    </span>
                  </div>
                </div>
        ...
        <Dialog open={eliteOpen} onOpenChange={setEliteOpen}>
          <DialogContent
            className="max-w-2xl border-2 border-ink bg-card p-6 md:p-8 max-h-[85vh] overflow-y-auto text-ink"
            style={{ boxShadow: "10px 10px 0 0 var(--cyan-cta)" }}
          >
            <DialogHeader>
        ...
        <p className="mt-10 text-center text-sm text-ink/50 reveal">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-navy font-semibold underline underline-offset-4 hover:text-gold"
          >
            WhatsApp us
          </a>{" "}
          — we'll build a quote around your goals.
        </p>
      </div>
    </section>
  );
